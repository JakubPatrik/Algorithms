...
public class Percolation {
    ...
    public boolean isFull(int rowX, int colX) {
        ...
        int row = rowX-1;
        int col = colX-1;
        if (topParentOn && isOpen(rowX, colX)) return wuUf.find(grid[row][col]) == wuUf.find(topParent);
        return false;
    }
    ...
    public boolean percolates() {
        ...
        // return isFull(size, bottomParent%size);
    }

    public static void main(String[] args) {
        Percolation dedo = new Percolation(3);
        dedo.open(1, 3);       // X X O
        dedo.open(2, 1);       // O X O
        dedo.open(2, 3);       // O X O
        dedo.open(3, 3);
        ...
        System.out.println(dedo.isFull(3, 1));
